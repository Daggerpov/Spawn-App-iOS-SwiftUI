...
struct UnifiedMapViewRepresentable: UIViewRepresentable {
		...
		if #available(iOS 17.0, *) {
			...
			if regionChanged && isLocationChange {
				...
				DispatchQueue.main.async {
					...
					guard CLLocationCoordinate2DIsValid(self.region.center)
					else {
						return
					}
	...
	class Coordinator: NSObject, MKMapViewDelegate {
			...
			-> MKAnnotationView?
		{
			...
			if let activityAnnotation = annotation as? ActivityAnnotation {
				if let customImage = createCustomPinImage(
				...
				if let fallbackImage = createCustomPinImage(
					icon: "⭐️",
		...
		private func createCustomPinImage(icon: String, color: UIColor)
			-> UIImage?
		{
			...
			let size = CGSize(
				...
				height: circleDiameter + pointerHeight
			)
		...
		private func getActivityIcon(for activity: FullFeedActivityDTO)
			-> String
		{
			...
			if let activityTypeId = activity.activityTypeId,
				let activityType = AppCache.shared.activityTypes.first(where: {
					$0.id == activityTypeId
				}),
