		...
		VStack(spacing: 0) {
			...
			.overlay(
				RoundedRectangle(cornerRadius: 16)
					.stroke(
						universalPlaceHolderTextColor(from: themeService, environment: colorScheme), lineWidth: 0.50)
			)
							...
							Button(action: {
								...
								HStack {
									Text("Already on Spawn")
										.font(Font.custom("Onest", size: 16).weight(.medium))
										.foregroundColor(
											universalPlaceHolderTextColor(from: themeService, environment: colorScheme))
								...
								ForEach(groupedRegularContacts, id: \.0) { letter, contactsInSection in
									VStack(alignment: .leading, spacing: 8) {
										...
										HStack {
											...
											Rectangle()
												.fill(
													universalPlaceHolderTextColor(
														from: themeService, environment: colorScheme
													).opacity(0.3)
												)
												.frame(height: 1)
										}
				...
				.background(
					LinearGradient(
						gradient: Gradient(colors: [
							universalBackgroundColor(from: themeService, environment: colorScheme).opacity(0),
