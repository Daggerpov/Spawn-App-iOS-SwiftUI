...
struct ActivityCreationView: View {
	...
	init(
		creatingUser: BaseUserDTO, closeCallback: @escaping () -> Void, selectedTab: Binding<TabType>,
		startingStep: ActivityCreationStep = .activityType
	) {
		...
		self._sourceTab = State(
			initialValue: hasPreselectedType && selectedTab.wrappedValue == .activities
				? .home : selectedTab.wrappedValue)

		// If an activity type is pre-selected and we're starting at activityType step,
		// skip directly to dateTime step
		let initialStep =
			(startingStep == .activityType && ActivityCreationViewModel.shared.selectedActivityType != nil)
			? .dateTime : startingStep
