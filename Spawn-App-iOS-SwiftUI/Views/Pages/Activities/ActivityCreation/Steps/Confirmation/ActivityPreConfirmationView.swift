...
struct ActivityPreConfirmationView: View {
	...
	var body: some View {
		VStack(spacing: 0) {
			...
			VStack(spacing: 0) {
				...
				// Activity title
				Text(
					viewModel.activity.title?.isEmpty == false
						? viewModel.activity.title! : (viewModel.selectedActivityType?.title ?? "Morning Stroll")
				)
				...
				ActivityNextStepButton(
					title: viewModel.isCreatingActivity
						? (viewModel.isEditingExistingActivity ? "Updating..." : "Creating...")
						: (viewModel.isEditingExistingActivity ? "Update Activity" : "Looks good to me!"),
	...
	private var activityCardView: some View {
		VStack(spacing: 8) {
			...
			// Activity details
			VStack(spacing: 2) {
				Text(viewModel.selectedActivityType?.title ?? "Spawn")
