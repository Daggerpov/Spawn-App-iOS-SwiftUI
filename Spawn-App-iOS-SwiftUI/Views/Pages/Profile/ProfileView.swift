...
extension ProfileView {
	private var weeklyCalendarView: some View {
		VStack(spacing: 8) {
			...
			// Days of week header
			HStack(spacing: 0) {
				ForEach(Array(zip(0..<weekDays.count, weekDays)), id: \.0) {
					index,
					day in
					Text(day)
						.font(.caption2)
					...
					ForEach(0..<5, id: \.self) { row in
						HStack(spacing: 6) {
							ForEach(0..<7, id: \.self) { col in
								if let dayActivities = getDayActivities(
									row: row,
									col: col
								) {
	...
	private func showDayEvents(activities: [CalendarActivityDTO]) {
		...
		if let presentationController = sheet.presentationController
	...
	private func getDayActivities(row: Int, col: Int) -> [CalendarActivityDTO]?
	{
		...
		if let firstActivity = activity {
			...
			return profileViewModel.allCalendarActivities.filter { act in
				...
				let actMonth = Calendar.current.component(
					.month,
					from: act.date
				)
