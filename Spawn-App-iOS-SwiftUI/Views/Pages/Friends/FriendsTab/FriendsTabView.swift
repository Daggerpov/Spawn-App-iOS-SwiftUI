		...
		ZStack {
			...
			.task {
				...
				let hasCachedData = await MainActor.run {
					...
					return !viewModel.friends.isEmpty || !viewModel.recommendedFriends.isEmpty
						|| !viewModel.incomingFriendRequests.isEmpty || !viewModel.outgoingFriendRequests.isEmpty
				}
			...
			.refreshable {
				...
				// Use forceRefresh to bypass cache and fetch fresh data from API
						...
						!blockReason.isEmpty
					{
						Task {
							await viewModel.blockUser(
								blockerId: currentUserId, blockedId: friendToBlock.id, reason: blockReason)
							blockReason = ""
						}
					}
				}
			...
			.alert("Remove Friend", isPresented: $showRemoveFriendConfirmation) {
				Button("Remove", role: .destructive) {
					...
					{
						Task {
							await viewModel.removeFriendAndRefresh(
								currentUserId: currentUserId, friendUserId: friendToRemove.id)
						}
					}
				}
				Button("Cancel", role: .cancel) {}
