...
struct ActivityFeedView: View {
	...
	init(
		...
		deepLinkedActivityId: Binding<UUID?> = .constant(nil),
		shouldShowDeepLinkedActivity: Binding<Bool> = .constant(false)
	) {
		self.user = user
	...
	var body: some View {
		...
		.onChange(of: shouldShowDeepLinkedActivity) { _, shouldShow in
			print(
				"🔗 ActivityFeedView onChange: shouldShowDeepLinkedActivity changed to \(shouldShow), activityId: \(deepLinkedActivityId?.uuidString ?? "nil")"
			)
			...
			do {
				...
				if let existingActivity = viewModel.activities.first(where: { $0.id == activityId }) {
					...
					await MainActor.run {
						...
						print(
							"🎯 ActivityFeedView: Set popup state for existing activity - showing: \(showingActivityPopup), activity: \(existingActivity.title ?? "No title")"
						)
					}
					return
				}
