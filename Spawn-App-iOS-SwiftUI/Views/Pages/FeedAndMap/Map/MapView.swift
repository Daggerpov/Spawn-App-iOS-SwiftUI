		...
		ZStack {
			...
			.task {
				...
				await MainActor.run {
					if let userLocation = locationManager.userLocation {
						...
						print(
							"✅ [NAV] MapView: Set initial region to activities"
						)
					}
				}
			}
			...
			.onChange(of: locationManager.locationUpdated) {
				...
				guard isViewVisible else {
					print(
						"⚠️ [NAV] MapView: Ignoring location update - view not visible"
					)
					return
				}
			...
			.onChange(of: viewModel.activities) {
				...
				guard isViewVisible else {
					print(
						"⚠️ [NAV] MapView: Ignoring activities update - view not visible"
					)
					return
				}
			...
			.onChange(of: locationManager.locationError) { _, error in
				...
				guard isViewVisible else {
					print(
						"⚠️ [NAV] MapView: Ignoring location error - view not visible"
					)
					return
				}
