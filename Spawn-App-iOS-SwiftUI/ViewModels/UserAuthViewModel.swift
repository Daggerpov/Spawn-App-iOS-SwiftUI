...
class UserAuthViewModel: NSObject, ObservableObject {
	...
	func signInWithGoogle() async {
		await MainActor.run {
			...
			_ = GIDConfiguration(
				clientID:
					"822760465266-1dunhm4jgrcg17137rfjo2idu5qefchk.apps.googleusercontent.com"
			)
			...
			GIDSignIn.sharedInstance.signIn(
				...
				// Get ID token
				signInResult.user.refreshTokensIfNeeded {
					[weak self] user, error in
					guard let self = self else { return }
					guard error == nil else {
						print(
							"Error refreshing token: \(error?.localizedDescription ?? "Unknown error")"
						)
						return
					}
		...
		if let url = URL(string: APIService.baseURL + "auth/sign-in") {
			...
			do {
				let parameters: [String: String] = [
					"idToken": unwrappedIdToken, "email": emailToUse,
					"provider": unwrappedProvider.rawValue,
				]
	...
	private func handleApiError(_ error: APIError) {
		...
		if case .invalidStatusCode(let statusCode) = error, statusCode == 404 {
			...
			self.shouldNavigateToUserInfoInputView = true
			print(
				"User does not exist yet in Spawn database - directing to user info input"
			)
		} else {
			self.spawnUser = nil
			self.shouldNavigateToUserInfoInputView = true
	...
	func spawnMakeUser(
		...
		if profilePicture == nil, let profilePicUrl = self.profilePicUrl,
			!profilePicUrl.isEmpty
		{
			parameters["profilePicUrl"] = profilePicUrl
			print(
				"Including provider picture URL in parameters: \(profilePicUrl)"
			)
		}
	...
	func deleteAccount() async {
		...
		if let url = URL(string: APIService.baseURL + "users/\(userId)") {
			do {
				...
				try await self.apiService.deleteData(
					from: url,
					parameters: nil,
					object: EmptyObject()
				)
	...
	func spawnFetchDefaultProfilePic() async {
		if let url = URL(string: APIService.baseURL + "users/default-pfp") {
			do {
				let fetchedDefaultPfpUrlString: String =
	...
	func updateProfilePicture(_ image: UIImage) async {
		...
		if let user = spawnUser {
			print(
				"Starting profile picture update for user \(userId) (username: \(user.username), name: \(user.name ?? "")) with image data size: \(imageData.count) bytes"
			)
		} else {
			print(
				"Starting profile picture update for user \(userId) with image data size: \(imageData.count) bytes"
			)
		}
		...
		do {
			...
			if let apiService = apiService as? APIService {
				let updatedUser = try await apiService.updateProfilePicture(
					imageData,
					userId: userId
				)
				...
				await MainActor.run {
					...
					print(
						"Profile successfully updated with new picture: \(updatedUser.profilePicture ?? "nil")"
					)
				}
				return
			}
			...
			if let url = URL(
				string: APIService.baseURL + "users/update-pfp/\(userId)"
			) {
				...
				print(
					"Fallback: Sending profile picture update request to: \(url)"
				)
				...
				if let updatedUser = try? decoder.decode(
					BaseUserDTO.self,
					from: data
				) {
					await MainActor.run {
					...
					print(
						"Failed to decode user data after profile picture update"
					)
				}
			}
		} catch {
			print(
				"Error updating profile picture: \(error.localizedDescription)"
			)
		}
	}
	...
	func spawnEditProfile(username: String, name: String) async {
		...
		if let user = spawnUser {
			print(
				"Editing profile for user \(userId) (username: \(user.username), name: \(user.name ?? ""))"
			)
		}
		...
		if let url = URL(string: APIService.baseURL + "users/update/\(userId)")
		{
			do {
				...
				print(
					"Updating profile with: username=\(username), name=\(name)"
				)

				let updatedUser: BaseUserDTO = try await self.apiService
					.patchData(
						from: url,
						with: updateDTO
					)
				...
				await MainActor.run {
					...
					print(
						"Profile updated successfully: \(updatedUser.username)"
					)
				}
			} catch {
		...
		if let url = URL(string: APIService.baseURL + "users/\(userId)") {
			do {
				...
				await MainActor.run {
					...
					print(
						"User data refreshed: \(updatedUser.username), \(updatedUser.name ?? "")"
					)
				}
			} catch {
			...
			do {
				...
				if !result {
					throw NSError(
						...
						code: 400,
						userInfo: [
							NSLocalizedDescriptionKey: "Password change failed"
						]
					)
				}
	...
	func quickSignIn() async {
		...
		do {
			if let url: URL = URL(
				string: APIService.baseURL + "auth/quick-sign-in"
			) {
				let user: BaseUserDTO = try await self.apiService.fetchData(
					from: url,
					parameters: nil
				)
