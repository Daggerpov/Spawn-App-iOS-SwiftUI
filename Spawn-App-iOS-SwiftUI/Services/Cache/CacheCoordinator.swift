...
class CacheCoordinator: ObservableObject {
	...
	func validateCache() async {
		...
		if allTimestamps.isEmpty {
			...
			let _ = await (
				friendsTask, activitiesTask, activityTypesTask, recommendedFriendsTask, friendRequestsTask,
				sentFriendRequestsTask
			)
			...
			await MainActor.run {
				...
				if let friendsResponse = result[CacheKeys.friends], friendsResponse.invalidate {
					if let updatedItems = friendsResponse.updatedItems,
						let updatedFriends = try? JSONDecoder().decode(
							[UUID: [FullFriendUserDTO]].self, from: updatedItems)
					{
				...
				if let activitiesResponse = result[CacheKeys.events], activitiesResponse.invalidate {
					if let updatedItems = activitiesResponse.updatedItems,
						let updatedActivities = try? JSONDecoder().decode(
							[UUID: [FullFeedActivityDTO]].self, from: updatedItems)
					{
				...
				// Recommended Friends
				if let recommendedFriendsResponse = result[CacheKeys.recommendedFriends],
					recommendedFriendsResponse.invalidate
				{
					if let updatedItems = recommendedFriendsResponse.updatedItems,
						let updatedRecommendedFriends = try? JSONDecoder().decode(
							[UUID: [RecommendedFriendUserDTO]].self, from: updatedItems)
					{
				...
				if let friendRequestsResponse = result[CacheKeys.friendRequests], friendRequestsResponse.invalidate {
					if let updatedItems = friendRequestsResponse.updatedItems,
						let updatedFriendRequests = try? JSONDecoder().decode(
							[UUID: [FetchFriendRequestDTO]].self, from: updatedItems)
					{
				...
				if let sentFriendRequestsResponse = result[CacheKeys.sentFriendRequests],
					sentFriendRequestsResponse.invalidate
				{
					if let updatedItems = sentFriendRequestsResponse.updatedItems,
						let updatedSentFriendRequests = try? JSONDecoder().decode(
							[UUID: [FetchSentFriendRequestDTO]].self, from: updatedItems)
					{
				...
				if needsFriendsRefresh || needsActivitiesRefresh || needsActivityTypesRefresh
					|| needsOtherProfilesRefresh || needsRecommendedFriendsRefresh || needsFriendRequestsRefresh
					|| needsSentFriendRequestsRefresh
				{
					...
					Task {
						async let friendsTask: () =
							needsFriendsRefreshCapture ? friendshipCache.refreshFriends() : noop()
						async let activitiesTask: () =
							needsActivitiesRefreshCapture ? activityCache.refreshActivities() : noop()
						async let activityTypesTask: () =
							needsActivityTypesRefreshCapture ? activityCache.refreshActivityTypes() : noop()
						async let otherProfilesTask: () =
							needsOtherProfilesRefreshCapture ? profileCache.refreshOtherProfiles() : noop()
						async let recommendedFriendsTask: () =
							needsRecommendedFriendsRefreshCapture ? friendshipCache.refreshRecommendedFriends() : noop()
						async let friendRequestsTask: () =
							needsFriendRequestsRefreshCapture ? friendshipCache.refreshFriendRequests() : noop()
						async let sentFriendRequestsTask: () =
							needsSentFriendRequestsRefreshCapture ? friendshipCache.refreshSentFriendRequests() : noop()

						let _ = await (
							friendsTask, activitiesTask, activityTypesTask, otherProfilesTask, recommendedFriendsTask,
							friendRequestsTask, sentFriendRequestsTask
						)
					}
				}
			}
	...
	func refreshAllProfilePictures() async {
		...
		if let userFriendRequests = friendshipCache.friendRequests[userId] {
			for request in userFriendRequests {
				usersToRefresh.append(
					(userId: request.senderUser.id, profilePictureUrl: request.senderUser.profilePicture))
			}
		}
		...
		// Remove duplicates
		var seen = Set<UUID>()
		let uniqueUsers = usersToRefresh.compactMap {
			(user: (userId: UUID, profilePictureUrl: String?)) -> (userId: UUID, profilePictureUrl: String?)? in
